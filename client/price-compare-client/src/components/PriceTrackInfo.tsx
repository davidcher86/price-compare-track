...
export const PriceTrackInfo: React.FC<PriceTrackInfoProps> = memo(({ priceTrackDetails, priceTrackResults }) => {
    ...
    if (!priceTrackDetails || !formattedInfo) {
        return (
            <div className="price-track-info-container bg-gradient-to-r from-slate-900 to-slate-800 backdrop-blur-md border border-slate-700 rounded-xl p-6 mx-4 mb-4 shadow-sm">
                <div className="flex flex-col items-center justify-center py-8 text-slate-400">
                    ...
                    <h3 className="text-lg font-medium text-slate-300 mb-2">No Item Selected</h3>
    ...
    return (
        <div className="price-track-info-container bg-gradient-to-r from-slate-900 to-slate-800 backdrop-blur-md border border-slate-700 rounded-xl p-6 mx-4 mb-4 shadow-lg hover:shadow-xl transition-all duration-300">
            {/* Header Section */}
            <div className="flex items-start justify-between mb-6">
                <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3 mb-3">
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border ${formattedInfo.statusBadgeColor}`}>
                            <div className={`w-2 h-2 rounded-full mr-2 ${formattedInfo.isEnabled ? 'bg-green-400' : 'bg-red-400'}`}></div>
                    ...
                    <div className="flex items-center text-sm text-slate-300 mb-3">
                        <div className="flex items-center">
                            ...
                            <span className="ml-1 theme-font font-semibold text-blue-300">{priceTrackDetails.source}</span>
                        </div>
                        {priceTrackDetails.href && (
                            <button
                                onClick={handleExternalLinkClick}
                                className="ml-4 inline-flex items-center text-blue-400 hover:text-blue-300 transition-colors duration-200"
                ...
                <div className="product-image-container ml-6 flex-shrink-0">
                    <div className="w-20 h-20 rounded-lg overflow-hidden bg-slate-700 border border-slate-600 shadow-sm">
                        ...
                        <div className={`${priceTrackDetails.img ? 'hidden' : ''} w-full h-full bg-gradient-to-br from-slate-600 to-slate-700 flex items-center justify-center`}>
                            <svg className="w-8 h-8 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="current-price-section bg-slate-800/50 backdrop-blur-sm rounded-lg border border-slate-600 p-4 mb-6 shadow-sm">
                <div className="flex items-center justify-between">
                    <div>
                        <p className="text-sm font-medium text-slate-300 mb-1">Current Price</p>
                        <p className="text-2xl font-bold theme-font text-green-400">
                            {formattedInfo.displayPrice}
                        </p>
                    </div>
                    <div className="text-right">
                        <p className="text-sm font-medium text-slate-300 mb-1">Tracking Frequency</p>
            ...
            <div className="details-grid grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Created Date */}
                <div className="detail-item bg-slate-800/50 backdrop-blur-sm rounded-lg border border-slate-600 p-4">
                    ...
                    <p className="text-sm text-slate-100 font-medium">{formattedInfo.formattedCreatedDate}</p>
                </div>
                ...
                <div className="detail-item bg-slate-800/50 backdrop-blur-sm rounded-lg border border-slate-600 p-4">
                    <div className="flex items-center mb-2">
                        ...
                        <span className="text-sm font-medium text-slate-300">Last Checked</span>
                    </div>
                    <p className="text-sm text-slate-100 font-medium">{formattedInfo.formattedLastChecked}</p>
                </div>
            </div>
        </div>
    );
